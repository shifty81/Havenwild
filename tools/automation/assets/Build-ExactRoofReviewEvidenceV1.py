#!/usr/bin/env python3
"""Build deterministic W45C3A exact-roof review evidence from the pinned LPC mount.

This tool intentionally produces machine-local evidence only.  It never publishes
roof visuals.  The evidence bundle is designed to be uploaded/reviewed so exact
source regions can be promoted in W45C3B without guessing coordinates.
"""
...
OUT = Path("WORKSPACE/generated/structure_review/w45c3_exact_roof")
ZIP_OUT = Path("WORKSPACE/generated/structure_review/Havenwild_W45C3A_ExactRoofEvidence.zip")
CELL = 32
SOURCES = [
    ("roof.flat_shingle_a", "assets/source/licensed/lpc_revised/Structure/Roofing/Flat Shingle Roof A.png"),
    ("roof.gable_shingle_a", "assets/source/licensed/lpc_revised/Structure/Roofing/Gable Shingle Roof A.png"),
    ("roof.hipped_shingle_a", "assets/source/licensed/lpc_revised/Structure/Roofing/Hipped Shingle Roof A.png"),
    ("roof.adobe_stucco", "assets/source/licensed/lpc_revised/Structure/Roofing/Adobe Stucco Roof.png"),
    ("roof.adobe_brick", "assets/source/licensed/lpc_revised/Structure/Roofing/Adobe Brick Roof.png"),
    ("roof.trim", "assets/source/licensed/lpc_revised/Structure/Roofing/Roof Trim.png"),
    ("roof.chimney.brick_a", "assets/source/licensed/lpc_revised/Structure/Roofing/Brick Chimney A.png"),
]
ROLES = [
    "field", "north_eave", "south_eave", "west_rake", "east_rake",
    "ridge_horizontal", "ridge_vertical", "gable_west", "gable_east",
    "hip_nw", "hip_ne", "hip_sw", "hip_se",
    "valley_nw", "valley_ne", "valley_sw", "valley_se",
    ...
    "trim_repeat", "trim_end_left", "trim_end_right",
]
...
def main() -> int:
    ...
    for family, rel in SOURCES:
        p=root/rel
        (available if p.is_file() else missing).append((family,rel,p))
    if missing and args.require_all:
        raise SystemExit("W45C3A pinned roof source missing: " + ", ".join(rel for _,rel,_ in missing))
    if not available:
        print("W45C3A exact roof evidence deferred: pinned LPC source mount is unavailable")
    ...
    sources=[]
    for family, rel, path in available:
        image=Image.open(path).convert("RGBA")
        cells=analyze_cells(image)
        board_rel=OUT/f"{family.replace('.','_')}_grid_2x.png"
        make_board(image,root/board_rel)
        cell_rel=OUT/f"{family.replace('.','_')}_cells.json"
        payload={
            "schema":"havenwild.roof_source_cell_evidence.v1","pass":"167Z109W45C3A",
            "family":family,"sourcePath":rel,"sourceSha256":sha256_file(path),
            "sourceSize":[image.width,image.height],"grid":[math.ceil(image.width/CELL),math.ceil(image.height/CELL)],"cellSize":[CELL,CELL],
            "cells":cells,"duplicatePixelGroups":duplicate_groups(cells),
            "alphaEdgeConnectedGroups":connectivity_groups(cells),
            "warning":"Connectivity and duplicate groups are review aids only; they do not assign semantic roof topology roles."
        }
        (root/cell_rel).write_text(json.dumps(payload,indent=2)+"\n",encoding="utf-8")
        sources.append({"family":family,"sourcePath":rel,"sourceSha256":payload["sourceSha256"],"sourceSize":payload["sourceSize"],"grid":payload["grid"],"board":board_rel.as_posix(),"cellEvidence":cell_rel.as_posix(),"nonEmptyCells":sum(1 for c in cells if c['nonEmpty'])})
    manifest={
        "schema":"havenwild.exact_roof_review_evidence.v1","pass":"167Z109W45C3A",
        "sourceCommit":"f07f7f5892e67c932c68f70bb04472f2c64e46bc","cellSize":[CELL,CELL],
        "sources":sources,"missingSources":[rel for _,rel,_ in missing],
        "reviewRule":"Human visual review assigns topology roles. Cell analysis never auto-promotes roof art."
    }
    (outroot/'manifest.json').write_text(json.dumps(manifest,indent=2)+"\n",encoding="utf-8")
    selection={
        "schema":"havenwild.roof_exact_region_selection_template.v1","pass":"167Z109W45C3A",
        "instructions":[
            ...
            "Accepted selections are copied into a source-controlled W45C3B publication manifest; this machine-local template is not authority."
        ],
        "selections":[{"role":role,"family":None,"sourceRect":None,"cellIds":[],"reviewNote":""} for role in ROLES]
    }
    (outroot/'roof_selection_template_v1.json').write_text(json.dumps(selection,indent=2)+"\n",encoding="utf-8")
    ...
    print(f"W45C3A exact roof review evidence: {len(sources)}/{len(SOURCES)} source sheet(s)")
