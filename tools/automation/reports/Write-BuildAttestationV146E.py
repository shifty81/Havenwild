...
import argparse, datetime as dt, hashlib, json, platform, subprocess
...
def main():
    ...
    contracts=['Cargo.lock','content/build/validator_registry_v4.json','content/build/validation_profiles_v4.json','content/build/generated_output_registry_v2.json','content/build/validation_error_codes_v1.json']
    payload={'schema':'havenwild.build.provenance.v1','classification':'verified' if verified else 'candidate','createdAt':dt.datetime.now(dt.timezone.utc).isoformat(),'platform':platform.platform(),'toolchain':{'python':platform.python_version(),'cargo':version(['cargo','--version']),'rustc':version(['rustc','--version'])},'contracts':{p:sha(ROOT/p) for p in contracts},'buildReport':str(report.relative_to(ROOT)) if report.is_file() and report.is_relative_to(ROOT) else (str(report) if report else None),'buildReportSha256':sha(report) if report else None,'archive':str(a.archive) if a.archive else None,'archiveSha256':sha(a.archive) if a.archive else None,'gates':{'cleanExtraction':verified,'generation':verified,'validation':verified,'cargoCheck':verified,'clippy':verified,'tests':verified,'applicationBuild':verified}}
