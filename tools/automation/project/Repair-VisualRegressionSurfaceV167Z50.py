...
REVISION = "167Z50-format-independent-visual-regression-repair-v1"
...
def repair_preview_draw(changes: list[str]) -> None:
    ...
    geometry_pattern = re.compile(
        r"let\s+frame\s*=\s*Rect::new\(\s*column\s*\*\s*64\.0\s*,\s*"
        r"row\s*\*\s*64\.0\s*,\s*64\.0\s*,\s*64\.0\s*,?\s*\)\s*;.*?"
        r"let\s+params\s*=\s*\|\|\s*DrawTextureParams\s*\{.*?\}\s*;",
        re.S,
    )
    ...
    if "row * 96.0" not in updated:
        updated, count = geometry_pattern.subn(new, updated, count=1)
        if count != 1:
