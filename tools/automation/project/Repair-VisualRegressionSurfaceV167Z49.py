...
REVISION = "167Z49-title-creator-nature-visual-regression-repair-v1"
...
def repair_preview_draw(changes: list[str]) -> None:
    ...
    fn body_family(appearance: &CharacterAppearance) -> &'static str {
        if variant(appearance, "body/base") == Some("female_neutral") {
            "female"
        } else {
            "male"
        }
    ...
    fn compatible(&self, appearance: &CharacterAppearance, id: &str) -> Option<&Texture2D> {
        if Self::body_family(appearance) == "female" {
            let female_id = format!("{id}_female");
            if let Some(texture) = self.layers.get(&female_id) {
                return Some(texture);
            }
        }
        self.get(id)
    ...
    fn body(&self, appearance: &CharacterAppearance) -> Option<&Texture2D> {
        self.get(if Self::body_family(appearance) == "female" {
            "body_female"
        } else {
            "body_male"
        })
    ...
    old = '''        let frame = Rect::new(column * 64.0, row * 64.0, 64.0, 64.0);
        let size = area.h.min(area.w).min(224.0);
        let x = area.x + (area.w - size) * 0.5;
        let y = area.y + (area.h - size) * 0.5;
        let params = || DrawTextureParams {
            source: Some(frame),
            dest_size: Some(vec2(size, size)),
            ..Default::default()
        };'''
    new = '''        // Generated Universal LPC runtime caches use 64x96 frames.
        // The prior square 64x64 preview sampled only the top two thirds of
        // every body/clothing sheet, so the creator appeared undressed even
        // though the in-game compositor loaded the same layers correctly.
    ...
    elif "row * 96.0" not in updated:
        raise ValueError("creator preview frame geometry was not recognized")

    updated = updated.replace(
        'if let Some(texture) = layers.get("eyes") {',
...
pub(crate) fn draw_main_menu_shell(status: &str) {
    ...
    .enumerate()
    {
        draw_circle(left.x + 24.0, left.y + 63.0 + index as f32 * 49.0, 3.5, Color::from_rgba(185, 139, 73, 255));
        draw_text(line, left.x + 36.0, left.y + 69.0 + index as f32 * 49.0, 15.0, Color::from_rgba(225, 219, 199, 255));
    }
    draw_text(
    ...
    draw_text("LPC FOUNDATION ONLINE", right.x + 16.0, right.y + 66.0, 16.0, Color::from_rgba(194, 222, 156, 255));
    draw_text("Terrain, characters, clothing,", right.x + 16.0, right.y + 94.0, 14.0, Color::from_rgba(219, 213, 195, 255));
    draw_text("tools, structures, and nature are", right.x + 16.0, right.y + 116.0, 14.0, Color::from_rgba(219, 213, 195, 255));
    draw_text("routed through the pinned LPC base.", right.x + 16.0, right.y + 138.0, 14.0, Color::from_rgba(219, 213, 195, 255));
    draw_line(right.x + 16.0, right.y + 164.0, right.x + right.w - 16.0, right.y + 164.0, 1.0, Color::from_rgba(104, 83, 57, 255));
    draw_text("WORLD CREATION", right.x + 16.0, right.y + 194.0, 16.0, Color::from_rgba(194, 222, 156, 255));
    draw_text("Land, water, biomes, settlements,", right.x + 16.0, right.y + 222.0, 14.0, Color::from_rgba(219, 213, 195, 255));
    draw_text("housing, and family rules are now", right.x + 16.0, right.y + 244.0, 14.0, Color::from_rgba(219, 213, 195, 255));
    draw_text("part of the world setup contract.", right.x + 16.0, right.y + 266.0, 14.0, Color::from_rgba(219, 213, 195, 255));
    draw_text("COMMUNITY / DISCORD", right.x + 16.0, right.y + right.h - 42.0, 13.0, Color::from_rgba(164, 181, 174, 255));
...
def repair_preview_loader_and_title_call(changes: list[str]) -> None:
    ...
    if "draw_main_menu_shell" not in updated.split("};", 3)[0:3].__str__():
        # Add to the existing grouped import without reforming the whole file.
        updated = updated.replace(
            "draw_character_preview, draw_color_selector, draw_ocean_background, draw_panel, draw_section_panel, CharacterPreviewTextures,",
            "draw_character_preview, draw_color_selector, draw_main_menu_shell, draw_ocean_background, draw_panel, draw_section_panel, CharacterPreviewTextures,",
            1,
        )
    ...
    if old_loop in updated:
        updated = updated.replace(old_loop, new_loop, 1)
    elif "let female_id = format!" not in updated:
...
def repair_object_footprints(changes: list[str]) -> None:
    ...
    old = '''            ObjectKind::Bush
            ...
            | ObjectKind::Herb
            | ObjectKind::Lamp
            | ObjectKind::Sign => ObjectFootprint {
                blocks_movement: false,
                ..ObjectFootprint::single_tile()
            },'''
    new = '''            ObjectKind::Bush => ObjectFootprint {
    ...
    if old in updated:
        updated = updated.replace(old, new, 1)
    elif "ObjectKind::Bush => ObjectFootprint" not in updated:
