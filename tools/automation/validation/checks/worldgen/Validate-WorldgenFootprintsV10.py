...
pack = load_json('content/worldgen/packs/worldgen_home_island_v0_10.json')
if pack:
    check('pack:id', pack.get('id') == 'worldgen_home_island_v0_10', str(pack.get('id')))
    check('pack:version', pack.get('version') == '0.10.0', str(pack.get('version')))
...
editor = '\n'.join(path.read_text(errors='ignore') for path in (ROOT / 'crates/haven_editor/src').glob('*.rs'))
...
required_game = ['interaction_object_at', 'object.visual_rect()', 'is_cell_walkable']
for token in required_game:
    check(f'rust-game:{token}', token in game)
check('rust-game:object sort helper', 'object.sort_y()' in game or 'object_sort_y(*object)' in game)
check(
    'rust-game:WORLDGEN_PACK_PATH-v0-10',
    'WORLDGEN_PACK_PATH: &str = "content/worldgen/packs/worldgen_home_island_v0_10.json"' in game
    or 'WORLDGEN_PACK_PATH: &str = "content/worldgen/packs/worldgen_home_island_v0_10.json"' in game,
)
...
(logs / 'worldgen_footprint_validation_v0_10_report.json').write_text(json.dumps(report, indent=2))
